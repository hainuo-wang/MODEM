...
class ImageCleanModelStage2(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageCleanModelStage2, self).__init__(opt)
        ...
        self.net_deg = define_network(deepcopy(opt['network_Deg']))
        # self.net_deg = torch.compile(self.net_deg)
        self.net_deg = self.model_to_device(self.net_deg)
        self.load_network(self.net_deg, self.opt['path']['pretrain_network_Deg'], param_key=self.opt['path'].get('param_key', 'params'))
        if hasattr(self.net_deg, 'module'):
            self.model_deg = self.net_deg.module.E
        else:
            self.model_deg = self.net_deg.E
        # self.model_deg = torch.compile(self.model_deg)
        self.print_network(self.model_deg)
        self.model_deg.eval()
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('kl_opt'):
            kl_type = train_opt['kl_opt'].pop('type')
            cri_kl_cls = getattr(loss_module, kl_type)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, kl_s = self.net_g(self.lq)
        with torch.no_grad():
            _, _, kl_t = self.model_deg(torch.cat([self.lq, self.gt], dim=1))
        ...
        l_kl = self.cri_kl(kl_t, kl_s)
        loss_dict['l_kl'] = l_kl

        loss_total = l_pix + l_pear + l_kl
        loss_total.backward()
    ...
    def _log_validation_metric_values(self, current_iter, dataset_name,
        ...
        if tb_logger:
            for metric, value in self.metric_results.items():
                tb_logger.add_scalar(f'metrics/{metric}/{dataset_name}', value, current_iter)
