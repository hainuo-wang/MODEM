...
class ImageCleanModelStage1(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(ImageCleanModelStage1, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['train']['use_grad_clip']:
            if self.opt['train'].get('max_norm', None) is not None:
                torch.nn.utils.clip_grad_norm_(self.net_g.parameters(), self.opt['train']['max_norm'],
    ...
    def pad_test(self, window_size):
        ...
        gt_img = F.pad(self.gt, (0, mod_pad_w, 0, mod_pad_h), 'reflect')
        self.nonpad_test(img, gt_img)
        _, _, h, w = self.output.size()
        self.output = self.output[:, :, 0:h - mod_pad_h * scale, 0:w - mod_pad_w * scale]

    def nonpad_test(self, img=None, gt_img=None):
        if img is None:
            img = self.lq
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                pred = self.net_g_ema(img, gt_img)
